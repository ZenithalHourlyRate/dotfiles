...
if [ -z "$TMUX" ]; then
    if [[ "$SSH_CONNECTION" != "" ]]; then
        ...
        elif [[ "x$(echo $SSH_CONNECTION|cut -d' ' -f1)" == "x2001:470:fceb:3::1" ]]; then
            tmux attach -t zenith || tmux new -s zenith
        elif [[ "x$(echo $SSH_CONNECTION|cut -d' ' -f1)" == "x2001:470:fceb:12::1" ]]; then
            tmux attach -t capella || tmux new -s capella
        fi
    else
